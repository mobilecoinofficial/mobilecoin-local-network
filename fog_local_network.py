...
class FogNetwork(Network):
    def start(self):
        ...
        #                          encoding='utf8', shell=True).strip()
        # print(f"chain path = {chain}, key path = {key}, root path = {root}")

        root = os.path.join(PROJECT_DIR, 'ok_rsa_head.pem')
        chain = os.path.join(PROJECT_DIR, 'ok_rsa_chain_25519_leaf.pem')
        key = os.path.join(PROJECT_DIR, 'ok_rsa_chain_25519_leaf.key')
        ...
        peers = [f'--peer "insecure-mc://localhost:{node.client_port}/"' for node in self.nodes]

        cmd = ' '.join([
            f'{TARGET_DIR}/fog-distribution',
            f'--sample-data-dir {PROJECT_DIR}/sample_data',
            '--num-tx-to-send 1',
        ] + peers)

        print('Performing Fog Distribution', cmd)
        print()
...
if __name__ == '__main__':
    ...
    FogNetwork().default_entry_point(args.network_type, args.block_version)
