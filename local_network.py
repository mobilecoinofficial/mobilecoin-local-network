...
LEDGER_BASE = os.path.join(PROJECT_DIR, 'sample_data', 'ledger')

TARGET_DIR = os.path.join(PROJECT_DIR, 'bin')
WORK_DIR = os.path.join(PROJECT_DIR, '__work__')
...
class Node:
    def __init__(self, name, node_num, client_port, peer_port, admin_port, admin_http_gateway_port, peers, quorum_set, block_version):
        ...
        self.localstack_bucket = f'node-{self.node_num}-ledger'
    ...
    def start(self, network):
        ...
        tx_source_urls = [f's3://{node.localstack_host}:{node.localstack_port}/{node.localstack_bucket}' for node in network.nodes if node.name in peer_names]
        ...
        cmd = ' '.join([
            f'aws --endpoint-url=http://{self.localstack_host}:{self.localstack_port} s3 mb s3://{self.localstack_bucket}',
        ])
        print(f'Creating localstack bucket for node: {cmd}')
        ...
        cmd = ' '.join([
            ...
            f'--ledger-path {self.ledger_dir}',
            f'--dest "s3://{self.localstack_bucket}?region=http://{self.localstack_host}:{self.localstack_port}"',
...
class Mobilecoind:
    ...
    def start(self, network):
        ...
        tx_srcs = [f'--tx-source-url "http://{node.localstack_host}:{node.localstack_port}/{node.localstack_bucket}"' for node in network.nodes]
